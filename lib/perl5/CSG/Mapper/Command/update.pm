...
sub opt_spec {
  return (
    ...
    ['fastq=s',             'Update the fastq record'],
    ['reads=i',             'Number of reads found in fastq'],
  );
}
...
sub validate_args {
  ...
  if ($opts->{fastq}) {
    unless (-e $opts->{fastq}) {
      $self->usage_error('fastq does not exist');
    }
  }
...
sub execute {
  ...
  if ($opts->{fastq}) {
    my $fastq = $schema->resultset('Fastq')->find({path => $opts->{fastq}});
    unless ($fastq) {
      $logger->critical("Unable to locate fastq $opts->{fastq}");
      exit 1;
    }

    unless ($fastq->sample->id == $meta->result->sample->id) {
      $logger->critical("fastq $opts->{fastq} belongs to another sample");
      exit 1;
    }

    if ($opts->{reads}) {
      $logger->info("updated reads to $opts->{reads} for fastq $opts->{fastq}");
      $fastq->update({read_cnt => $opts->{reads}});
    }
  }
}
